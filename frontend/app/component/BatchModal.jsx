    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl flex">
        ...
        <div className="w-2/3 pr-6 border-r">
          <h2 className="text-2xl font-bold mb-4 text-gray-800">Batch Details - {batch?.batchId}</h2>
          <div className="text-left space-y-2">
            ...
            <p><strong>Status:</strong> <span className={`font-semibold ${batch?.status === 'Completed' ? 'text-green-600' : 'text-yellow-600'}`}>{batch?.status}</span></p>
          </div>
        </div>
        
        {/* Right Side - QR Code */}
        <div className="w-1/3 flex flex-col items-center justify-center pl-6">
          {batch?.batchId ? <QRCodeBatch batchId={batch.batchId} /> : <p className="text-red-500">No QR Code Available</p>}
        </div>
      </div>
