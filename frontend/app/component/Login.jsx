...
function LoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = isSignup ? `http://localhost:5023/api/auth/register` : `http://localhost:5023/api/auth/login`;
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-900">
      ...
      >
        <div className="flex h-full">
          <motion.div className="w-1/2 p-4 flex flex-col items-center justify-center bg-navy-800/50 backdrop-blur-sm rounded-xl border border-white/20">
            <motion.img
              src="/Diamond.avif"
              alt="Company Logo"
              className="w-24 h-24 rounded-full mb-4 backdrop-blur-sm border-2 border-white/30"
              ...
              transition={{ duration: 1 }}
            />
            <h1 className="text-xl font-bold text-white">Diamond Management System</h1>
            <p className="text-center text-gray-300 text-sm mt-2">Secure access to your professional dashboard</p>
          </motion.div>

          <motion.div className="w-1/2 p-8 px-16 flex flex-col items-center justify-center">
            <motion.h1 className="text-2xl font-bold text-center text-white mb-6">{isSignup ? "Signup" : "Login"}</motion.h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 w-full">
              {isSignup && (
                <input
                  ...
                  value={formData.name}
                  onChange={handleChange}
                  className="w-full border bg-white/20 px-3 py-2 rounded-md focus:outline-none text-white"
                />
              )}
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-blue-500/90 text-white px-4 py-2 rounded-md flex items-center justify-center gap-2"
              >
                {isLoading ? <FiLoader className="animate-spin" /> : isSuccess ? <FiCheckCircle /> : <FiArrowRight />}
                {isSignup ? "Signup" : "Login"}
              </button>
            </form>
            ...
            <button
              onClick={() => setIsSignup(!isSignup)}
              className="text-blue-400 hover:text-blue-300 mt-4 text-sm font-medium"
            >
              {isSignup ? "Already have an account?" : "Don't have an account?"}
            </button>
          </motion.div>
